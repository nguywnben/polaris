...
const PROVIDER_FORM_LABEL_VALUES = {
    en: ['{provider} Advanced Settings', 'Upstream Endpoint', 'Upstream Endpoints', 'OAuth Client', 'Protocol Behavior', 'Authorization service', 'Token service', '{provider} Connection', '{provider} API Key', 'Optional', 'Containers cannot reach the host through their own localhost. Use the host gateway or a network-reachable endpoint when Omni Gateway runs in Docker.', 'Collect streaming responses for non-stream clients', 'Switch Google Antigravity credentials during retries', 'OAuth issuer', 'Usage API', 'Endpoint', 'OAuth client ID', 'Payload user agent'],
    'zh-CN': ['{provider} 高级设置', '上游端点', '上游端点', 'OAuth 客户端', '协议行为', '授权服务', '令牌服务', '{provider} 连接', '{provider} API 密钥', '可选', '在容器中，localhost 无法访问宿主机。Omni Gateway 运行于 Docker 时，请使用宿主机网关或网络可达的端点。', '为非流式客户端收集完整的流式响应', '重试时切换 Google Antigravity 凭据', 'OAuth 颁发方', '用量 API', '端点', 'OAuth 客户端 ID', '载荷 user agent'],
    'zh-TW': ['{provider} 進階設定', '上游端點', '上游端點', 'OAuth 用戶端', '協定行為', '授權服務', '權杖服務', '{provider} 連線', '{provider} API 金鑰', '選填', '容器中的 localhost 無法連線到主機。Omni Gateway 在 Docker 執行時，請使用主機閘道或網路可連線的端點。', '為非串流用戶端收集完整串流回應', '重試時切換 Google Antigravity 憑證', 'OAuth 發行者', '用量 API', '端點', 'OAuth 用戶端 ID', 'Payload user agent'],
    de: ['Erweiterte Einstellungen für {provider}', 'Upstream-Endpunkt', 'Upstream-Endpunkte', 'OAuth-Client', 'Protokollverhalten', 'Autorisierungsdienst', 'Token-Dienst', '{provider}-Verbindung', '{provider}-API-Schlüssel', 'Optional', 'Container erreichen den Host nicht über ihr eigenes localhost. Verwenden Sie das Host-Gateway oder einen erreichbaren Endpunkt, wenn Omni Gateway in Docker läuft.', 'Streaming-Antworten für Nicht-Streaming-Clients sammeln', 'Google-Antigravity-Zugangsdaten bei Wiederholungen wechseln', 'OAuth-Aussteller', 'Nutzungs-API', 'Endpunkt', 'OAuth-Client-ID', 'Payload-User-Agent'],
    es: ['Configuración avanzada de {provider}', 'Endpoint ascendente', 'Endpoints ascendentes', 'Cliente OAuth', 'Comportamiento del protocolo', 'Servicio de autorización', 'Servicio de tokens', 'Conexión de {provider}', 'Clave API de {provider}', 'Opcional', 'Los contenedores no pueden acceder al host mediante su propio localhost. Usa la puerta de enlace del host o un endpoint accesible por red cuando Omni Gateway se ejecute en Docker.', 'Recopilar respuestas en streaming para clientes sin streaming', 'Cambiar credenciales de Google Antigravity durante los reintentos', 'Emisor OAuth', 'API de uso', 'Endpoint', 'ID de cliente OAuth', 'User agent del payload'],
    fr: ['Paramètres avancés de {provider}', 'Endpoint en amont', 'Endpoints en amont', 'Client OAuth', 'Comportement du protocole', 'Service d’autorisation', 'Service de jetons', 'Connexion {provider}', 'Clé API {provider}', 'Facultatif', 'Un conteneur ne peut pas joindre l’hôte via son propre localhost. Utilisez la passerelle de l’hôte ou un endpoint joignable lorsque Omni Gateway s’exécute dans Docker.', 'Collecter les réponses en streaming pour les clients non streaming', 'Changer d’identifiant Google Antigravity lors des nouvelles tentatives', 'Émetteur OAuth', 'API d’utilisation', 'Endpoint', 'ID client OAuth', 'User agent du payload'],
    id: ['Pengaturan Lanjutan {provider}', 'Endpoint Upstream', 'Endpoint Upstream', 'Klien OAuth', 'Perilaku Protokol', 'Layanan otorisasi', 'Layanan token', 'Koneksi {provider}', 'Kunci API {provider}', 'Opsional', 'Container tidak dapat menjangkau host melalui localhost miliknya. Gunakan gateway host atau endpoint yang dapat dijangkau jaringan saat Omni Gateway berjalan di Docker.', 'Kumpulkan respons streaming untuk klien non-stream', 'Ganti kredensial Google Antigravity saat mencoba ulang', 'Penerbit OAuth', 'API penggunaan', 'Endpoint', 'ID klien OAuth', 'User agent payload'],
    it: ['Impostazioni avanzate di {provider}', 'Endpoint upstream', 'Endpoint upstream', 'Client OAuth', 'Comportamento del protocollo', 'Servizio di autorizzazione', 'Servizio token', 'Connessione {provider}', 'Chiave API {provider}', 'Facoltativo', 'I container non possono raggiungere l’host tramite il proprio localhost. Usa il gateway host o un endpoint raggiungibile quando Omni Gateway è in esecuzione su Docker.', 'Raccogli le risposte in streaming per i client non streaming', 'Cambia credenziale Google Antigravity durante i nuovi tentativi', 'Autorità OAuth', 'API di utilizzo', 'Endpoint', 'ID client OAuth', 'User agent del payload'],
    ja: ['{provider} の詳細設定', '上流エンドポイント', '上流エンドポイント', 'OAuth クライアント', 'プロトコル動作', '認証サービス', 'トークンサービス', '{provider} 接続', '{provider} API キー', '任意', 'コンテナからホスト自身の localhost には接続できません。Omni Gateway を Docker で実行する場合は、ホストゲートウェイまたはネットワークから到達可能な endpoint を使用してください。', '非ストリームクライアント向けにストリーム応答を収集する', '再試行時に Google Antigravity の認証情報を切り替える', 'OAuth 発行者', '使用量 API', 'エンドポイント', 'OAuth クライアント ID', 'ペイロード user agent'],
    ko: ['{provider} 고급 설정', '업스트림 endpoint', '업스트림 endpoint', 'OAuth 클라이언트', '프로토콜 동작', '인증 서비스', '토큰 서비스', '{provider} 연결', '{provider} API 키', '선택 사항', '컨테이너는 자체 localhost를 통해 호스트에 연결할 수 없습니다. Omni Gateway가 Docker에서 실행될 때는 호스트 게이트웨이나 네트워크로 연결 가능한 endpoint를 사용하세요.', '비스트리밍 클라이언트를 위해 스트리밍 응답 수집', '재시도 중 Google Antigravity 자격 증명 전환', 'OAuth 발급자', '사용량 API', 'Endpoint', 'OAuth 클라이언트 ID', 'Payload user agent'],
    pt: ['Configurações avançadas do {provider}', 'Endpoint upstream', 'Endpoints upstream', 'Cliente OAuth', 'Comportamento do protocolo', 'Serviço de autorização', 'Serviço de token', 'Conexão {provider}', 'Chave API {provider}', 'Opcional', 'Contêineres não acessam o host pelo próprio localhost. Use o gateway do host ou um endpoint acessível pela rede quando o Omni Gateway estiver no Docker.', 'Coletar respostas em streaming para clientes sem streaming', 'Alternar credenciais do Google Antigravity durante novas tentativas', 'Emissor OAuth', 'API de uso', 'Endpoint', 'ID do cliente OAuth', 'User agent do payload'],
    ru: ['Расширенные настройки {provider}', 'Вышестоящий endpoint', 'Вышестоящие endpoint', 'Клиент OAuth', 'Поведение протокола', 'Служба авторизации', 'Служба токенов', 'Подключение {provider}', 'API-ключ {provider}', 'Необязательно', 'Контейнер не может обратиться к хосту через собственный localhost. При запуске Omni Gateway в Docker используйте шлюз хоста или доступный по сети endpoint.', 'Собирать потоковые ответы для непотоковых клиентов', 'Менять учётные данные Google Antigravity при повторных попытках', 'Издатель OAuth', 'API использования', 'Endpoint', 'ID клиента OAuth', 'User agent payload'],
    th: ['การตั้งค่าขั้นสูงของ {provider}', 'Endpoint ต้นทาง', 'Endpoint ต้นทาง', 'ไคลเอนต์ OAuth', 'พฤติกรรมโปรโตคอล', 'บริการอนุญาต', 'บริการโทเค็น', 'การเชื่อมต่อ {provider}', 'คีย์ API ของ {provider}', 'ไม่บังคับ', 'คอนเทนเนอร์เข้าถึงโฮสต์ผ่าน localhost ของตนเองไม่ได้ เมื่อ Omni Gateway ทำงานใน Docker ให้ใช้ host gateway หรือ endpoint ที่เข้าถึงผ่านเครือข่ายได้', 'รวบรวมการตอบกลับแบบสตรีมสำหรับไคลเอนต์แบบไม่สตรีม', 'สลับข้อมูลรับรอง Google Antigravity ระหว่างการลองใหม่', 'ผู้ออก OAuth', 'API การใช้งาน', 'Endpoint', 'ID ไคลเอนต์ OAuth', 'User agent ของ payload'],
    tr: ['{provider} Gelişmiş Ayarları', 'Üst endpoint', 'Üst endpoint’ler', 'OAuth İstemcisi', 'Protokol Davranışı', 'Yetkilendirme hizmeti', 'Token hizmeti', '{provider} Bağlantısı', '{provider} API Anahtarı', 'İsteğe bağlı', 'Kapsayıcılar kendi localhost adresleri üzerinden ana makineye erişemez. Omni Gateway Docker’da çalışırken ana makine ağ geçidini veya ağdan erişilebilen bir endpoint kullanın.', 'Akışsız istemciler için akış yanıtlarını topla', 'Yeniden denemelerde Google Antigravity kimlik bilgisini değiştir', 'OAuth veren kuruluş', 'Kullanım API’si', 'Endpoint', 'OAuth istemci kimliği', 'Payload user agent'],
    vi: ['Cài đặt nâng cao của {provider}', 'Endpoint thượng nguồn', 'Các endpoint thượng nguồn', 'Ứng dụng OAuth', 'Hành vi giao thức', 'Dịch vụ cấp quyền', 'Dịch vụ token', 'Kết nối {provider}', 'Khóa API {provider}', 'Không bắt buộc', 'Container không thể truy cập máy chủ qua localhost của chính nó. Khi Omni Gateway chạy trong Docker, hãy dùng host gateway hoặc endpoint có thể truy cập qua mạng.', 'Thu thập phản hồi streaming cho client không streaming', 'Chuyển thông tin xác thực Google Antigravity trong các lần thử lại', 'Đơn vị phát hành OAuth', 'API mức sử dụng', 'Endpoint', 'Mã client OAuth', 'User agent trong payload']
};
...
const SETTINGS_ABOUT_WORKFLOW_MESSAGES = {
    en: {
        ...
        'about.sponsor_description': 'Help sustain development and maintenance of Omni Gateway.',
        'about.sponsor_link': 'Sponsor Omni Gateway on GitHub',
    ...
    vi: {
        ...
        'about.sponsor_description': 'Góp phần duy trì việc phát triển và bảo trì Omni Gateway.',
        'about.sponsor_link': 'Tài trợ Omni Gateway trên GitHub',
