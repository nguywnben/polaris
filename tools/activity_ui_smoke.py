...
def main():
    ...
    audit = {
        ...
        "actor_fingerprint": "a" * 20,
        "action": "config.update",
        "target_type": "configuration",
        "target_fingerprint": "b" * 20,
        "outcome": "succeeded",
    ...
    def evidence(route):
        ...
        if state["error"]:
            ...
            route.fulfill(
                json={
                    "events": [] if state["empty"] else [audit],
                    "page_size": 25,
                    "has_more": False,
                    "next_cursor": None,
                }
    ...
    with disposable_runtime() as base, sync_playwright() as p:
        ...
        context.route_web_socket(
            "**/api/logs/stream",
            lambda ws: ws.send("[2026-09-14 20:00:00] [INFO] synthetic completed req-fixture"),
        )
        ...
        try:
            ...
            for view in ("traces", "audit", "runtime"):
                ...
                for width in (1440, 1024, 768, 360, 320):
                    ...
                    assert page.locator(".activity-tabs").evaluate(
                        "el => el.scrollWidth <= el.clientWidth"
                    ), (view, width)
                    if width in (1440, 360):
                        page.screenshot(
                            path=str(output / f"{view}-{width}.png"),
                            full_page=True,
                            animations="disabled",
                        )
            ...
            page.screenshot(
                path=str(output / "dark-en-mobile.png"), full_page=True, animations="disabled"
            )
            assert not errors, errors
            print(
                "PASS: shared filters, keyboard tabs, failed/empty/populated responses, retry, trace detail and audit pivot, runtime, en/vi, 320–1440px"
            )
        finally:
            context.close()
            browser.close()
