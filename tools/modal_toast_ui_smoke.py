        ...
        try:
            ...
            for width, theme in ((1440, "light"), (360, "dark")):
                ...
                for dialog_id in (
                    ...
                    route = (
                        "/activity?view=audit"
                        if dialog_id == "auditDetailDialog"
                        else "/activity"
                        if dialog_id == "traceDetailDialog"
                        else "/identity"
                    )
                    ...
                    if not page.evaluate(
                        ...
                        failures.append(
                            f"{width}/{dialog_id}: opening a modal covered the existing toast"
                        )
                    page.evaluate(
                        """id => {
                    ...
                    if not page.evaluate(
                        "id => document.getElementById(id).contains(document.getElementById('statusSection'))",
                        dialog_id,
                    ):
                        failures.append(
                            f"{width}/{dialog_id}: live region is outside the active modal"
                        )
                ...
                for field_id in ("toastTestInput", "toastTestTextarea", "toastTestSelect"):
                    ...
                    page.wait_for_timeout(40)
                    if not page.locator("#toastTestAction").evaluate(
            ...
            page.evaluate(
                "document.getElementById('statusSection').remove(); showStatus('Synthetic notice', 'success')"
            )
            ...
            print(
                "PASS: all native modals, custom modal, 3 control types, both themes/widths; toast preserves focus"
            )
        finally:
            context.close()
            browser.close()
