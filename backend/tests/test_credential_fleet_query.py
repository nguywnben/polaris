...
class CredentialFleetQueryTests(unittest.TestCase):
    def test_api_key_hint_is_masked_at_the_public_boundary(self):
        ...
        credential = {
            "provider": "openai", "credential_type": "api_key", "api_key": key,
            "token": "oauth-secret", "refresh_token": "refresh-secret",
        }
        item = enrich_credential_summary(
            {"filename": "key.json", "api_key_hint": key}, credential,
            backend_type="sqlite", mode="primary",
        )
        ...
        for value, expected in [("short-key", "••••"), ("", None), (None, None), (123, None), ("invalid\nkey", None)]:
            with self.subTest(value=value):
                result = enrich_credential_summary(
                    {}, {**credential, "api_key": value}, backend_type="sqlite", mode="primary",
                )
                self.assertEqual(result.get("api_key_hint"), expected)
        oauth = enrich_credential_summary(
            {}, {**credential, "credential_type": "oauth"}, backend_type="sqlite", mode="primary",
        )
