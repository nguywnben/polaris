...
class OAuthCallbackTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_claude_callback_requires_explicit_credential_save(self):
        complete = AsyncMock(
            return_value={
                "action": "created",
        ...
        self.assertNotIn("Authentication Successful", body)
        complete.assert_not_awaited()
        accept_google.assert_not_called()

    async def test_claude_callback_without_code_or_with_error_never_completes(self):
        for query in ({"state": "claude-state"}, {"state": "claude-state", "error": "denied"}):
            with (
                patch.object(root, "is_claude_oauth_state", return_value=True),
                patch.object(root, "complete_claude_oauth", AsyncMock(), create=True) as complete,
            ):
                response = await root.serve_oauth_callback(_callback_request(**query))
