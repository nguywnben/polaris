...
Базовый профиль не требует внешних сервисов и хранит данные в `polaris-data`. Шаблон готовится к `1.0.0`; версия ещё не опубликована. До установки устраните конфликт со старыми одноимёнными тегами/образами по [контрольному списку выпуска](../releases/1.0.0-preparation.md). Используйте [процедуру обновления и отката](../updating.md); расширенные функции подключаются через `deploy/compose.advanced.yml`.
