...
[Простая установка через Docker](../docker-install.md) для Linux/amd64 не требует клонирования репозитория и ручного редактирования `.env`: одна команда, явное согласие на HTTP для VPS и создание пароля в браузере. **Подготовлено локально; сначала должны быть опубликованы соответствующие обновлённые установщик и образ.** Ручной Docker, Compose и запуск из исходников остаются доступны. Для Docker-run есть [отдельная инструкция по резервному копированию и обновлению](../docker-maintenance.md); инструмент обновления Compose к нему не применяется.
...
См. [контракт идентификаторов](../migrations/polaris.md) и [устранение неполадок](../troubleshooting.md). Нативные скрипты, Render и Zeabur — совместимые альтернативы без равноценной проверки установки/восстановления. Образы выпускаются для `linux/amd64`; выпуск `linux/arm64` приостановлен.
...
| `SETUP_ALLOW_INSECURE_HTTP` | `false` | Разрешает удалённую настройку по HTTP только при принятии риска незашифрованных учётных данных и сеансов. Рекомендуется HTTPS; надёжный токен настройки по-прежнему обязателен. |
...
Рекомендуется SQLite. Compose и установщик Docker сохраняют весь `/app/backend/data` в томе `polaris-data`. При ручном запуске Docker также монтируйте весь каталог данных: сохранения только `creds` и `logs` недостаточно для SQLite и настроек.
