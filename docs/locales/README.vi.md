...
## Tại sao nên chọn Polaris

Quy trình lập trình hiện đại thường kết hợp nhiều client và provider: công cụ tương thích OpenAI, SDK Gemini native, agent theo phong cách Anthropic, thông tin xác thực Google và các định tuyến mô hình thử nghiệm. Polaris đóng vai trò trung gian giữa các client đó và backend mô hình, giúp mỗi công cụ duy trì định dạng giao tiếp vốn có trong khi gateway xử lý việc định tuyến, thử lại (retry), dọn dẹp request và chuẩn hóa phản hồi.
...
| `XAI_USER_AGENT` | `grok-cli/polaris` | Ghi đè tùy chọn cho HTTP User-Agent chung dùng cho các yêu cầu Grok Build OAuth và SpaceXAI Console API. |
...
| `LOG_FILE` | `./backend/data/logs/polaris.log` | Đường dẫn file lưu trữ log. Trong Docker, lưu giữ bền vững `/app/backend/data/logs` bằng volume máy chủ. |
...
Khả năng tương thích Responses hỗ trợ văn bản, hình ảnh đầu vào, non-streaming function tools và SSE text streaming. Các công cụ tích hợp sẵn do OpenAI lưu trữ, lịch sử phản hồi lưu trữ và streaming function calls sẽ bị từ chối rõ ràng vì Polaris không thực thi, lưu trữ hoặc âm thầm loại bỏ những hành vi đặc thù riêng này của OpenAI.
...
Polaris cung cấp các route tương thích SDK mà không cần namespace phân biệt sản phẩm:
...
- Đặt Polaris phía sau một reverse proxy có bật TLS khi cho phép truy cập ngoài localhost.
...
Polaris được kế thừa và phát triển từ cộng đồng mã nguồn mở về định tuyến AI, telemetry và gateway. Chúng tôi bày tỏ lòng biết ơn sâu sắc đến những người sáng lập và duy trì các dự án sau:
...
Polaris được phát hành theo [Giấy phép MIT](../../LICENSE).
