...
[การติดตั้ง Docker แบบง่าย](../docker-install.md) สำหรับ Linux/amd64 ไม่ต้อง clone ที่เก็บโค้ดหรือแก้ `.env` เอง: รันหนึ่งคำสั่ง ยืนยันหากใช้ HTTP บน VPS แล้วสร้างรหัสผ่านผ่านเว็บ **ขณะนี้เตรียมไว้ในเครื่องเท่านั้น ต้องเผยแพร่ตัวติดตั้งและอิมเมจรุ่นปรับปรุงที่ตรงกันก่อน** ยังใช้ Docker แบบกำหนดเอง, Compose หรือรันซอร์สโค้ดได้ Docker-run มี[ขั้นตอนสำรองและอัปเดตแยกต่างหาก](../docker-maintenance.md) ไม่ใช้เครื่องมืออัปเดต Compose
...
ดู[ข้อตกลงชื่อระบุ](../migrations/polaris.md)และ[การแก้ปัญหา](../troubleshooting.md) สคริปต์ภายในเครื่อง Render และ Zeabur เป็นแนวทางที่ยังใช้งานร่วมกันได้ แต่ไม่ได้ผ่านการตรวจสอบติดตั้ง/กู้คืนเทียบเท่ากัน อิมเมจเผยแพร่สำหรับ `linux/amd64` ส่วน `linux/arm64` ยังระงับการเผยแพร่
...
| `SETUP_ALLOW_INSECURE_HTTP` | `false` | อนุญาตการตั้งค่าระยะไกลผ่าน HTTP เฉพาะเมื่อยอมรับความเสี่ยงที่ข้อมูลรับรองและเซสชันไม่ถูกเข้ารหัส แนะนำ HTTPS และยังต้องใช้โทเค็นตั้งค่าที่ปลอดภัย |
...
แนะนำให้ใช้ SQLite โดย Compose และตัวติดตั้ง Docker จะเก็บ `/app/backend/data` ทั้งหมดไว้ใน volume `polaris-data` หากใช้ Docker เอง ต้องเมานต์ทั้งโฟลเดอร์ข้อมูลเช่นกัน การเก็บเฉพาะ `creds` และ `logs` ไม่ครอบคลุม SQLite และการตั้งค่า
