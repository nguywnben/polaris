...
Le profil standard fonctionne sans service externe et conserve les données dans `polaris-data`. Le modèle épingle `0.1.0-beta.1`. Suivez le [guide de mise à jour et retour arrière](../updating.md). Activez ensuite les options de stockage externe, OIDC, proxy, protections, cache et télémétrie via `deploy/compose.advanced.yml`.
